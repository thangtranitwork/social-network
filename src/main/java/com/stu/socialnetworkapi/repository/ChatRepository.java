...
public interface ChatRepository extends Neo4jRepository<Chat, UUID> {
    @Query("""
            MATCH (u:User {id: $userId})-[:IS_MEMBER_OF]->(chat:Chat {type: "DIRECT"})<-[:IS_MEMBER_OF]-(target:User {id: $targetId})
            RETURN chat.id
            """)
    ...
    @Query("""
                        ...
                        // For DIRECT chat, get the other member (target)
                        ...
                        RETURN
                            ...
                            latestMessage.action AS latestMessageAction,
    ...
    @Query("""
            ...
            // === Filtering by query ===
            WITH chat, chatImage, latestMessage, sender, senderProfilePic,
